...
namespace AuctionAPI.Repositories
{
    public  class SellerRepository : Repository<Guid, Seller>
    {
        ...
        {
            var seller = await _auctionContext.Sellers.Where(s => s.User.Status != "Deleted")
                                                        .Include(s => s.User)
        ...
        {
            var sellers = _auctionContext.Sellers.Where(s => s.User.Status != "Deleted").Include(s=>s.User);
            if (sellers.Count() == 0)
